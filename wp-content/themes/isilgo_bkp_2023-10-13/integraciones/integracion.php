        ...
        if (trae_curso_regalado($id_actual, $current_user->user_email)) {
            ...
            if ($customer_orders) {
                ...
                foreach ($customer_orders as $customer_order) {
                    ...
                    $cursos_regalados = valida_orden_regalo($order->get_id());
                    foreach ($items as $item) {
                        $id_product = $item->get_product_id();
                        if (($order->get_status() == 'completed' || $order->get_status() == 'processing') && !in_array($id_product, $cursos_regalados) && !in_array($id_product, $membresias_id)) {
...
function tieneMembresia()
{
    ...
    if ($check_membresia || $fecha_actual <= $fecha_expiracion) {
