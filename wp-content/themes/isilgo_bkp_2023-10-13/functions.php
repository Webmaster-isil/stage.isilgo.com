            ...
            <?php foreach ($profesor as $profe) { ?>
                <li>
                    <?php if (get_field('foto_autor', 'user_' . $profe->ID)) : ?>
                        <div class="imagen_carrusel_profe" style="background:url('<?php echo get_field('foto_autor', 'user_' . $profe->ID); ?>')"></div>
                    <?php else : ?>
                        <div class="imagen_carrusel_profe" style="background:url('<?= get_template_directory_uri() ?>/assets/img/docentehold.png')"></div>
                    <?php endif; ?>
                    <div>                        
                        <p><strong><?php echo $profe->display_name; ?></strong></p>
                        <p><?php echo get_field('cargo', 'user_' . $profe->ID); ?></p>   
...
{ ?>
    <ul class="nav nav-tabs" id="tab_especial_membresia" role="tablist">
...
function envia_correo_regalo($registro)
{
    ...
    if (!get_post_meta($registro['id_producto'], 'RADAR_RECURRENCE_FREQ', true)) {
        $main_body = sprintf('%s te ha regalado un curso online "%s" puedes iniciar tu aprendizaje iniciando sesión <a href="%s">aquí</a>', $registro['nombre_remitente'], $nombre_curso, $full_url);
    } else {
        $main_body = sprintf('%s te ha regalado una membresía "%s" puedes potenciar tu perfil profesional desde ahora, inicia sesión <a href="%s">aquí</a>', $registro['nombre_remitente'], $nombre_curso, $full_url);
    }
    ...
    get_template_part('email-regalo', null, array(
        'nombre_destinatario' => $registro['nombre_destinatario'],
    ...
    ob_end_clean();
    $envio = wp_mail($registro['correo_destinatario'], 'Regalo Isilgo', $mensaje, array('Content-Type: text/html; charset=UTF-8'));
    log_functions('Envio de correo: ', var_export($envio), __FUNCTION__);
    if ($envio) {
...
function recientesVistos()
{
    ...
    $viewed_products = wp_parse_id_list((array) explode('|', wp_unslash($_COOKIE['woocommerce_recently_viewed'])));    
    if(count($id_carrito) > 0){ $muestreo = array_diff($viewed_products, $id_carrito); }
...
function cursosGratisHome()
{
    ...
    <div class="col-md-8 mx-auto">
        ...
        <div class="woocommerce cursos_gratis_desktop_home">
            <ul class="products woocommerce columns-2">
                <?php
...
function productosRecomendados()
{
    ...
    if (is_user_logged_in()) {
        ...
        if (count($categorias) > 0) {
            ...
            if ($product_count > 0) {
                echo '<h5 class="mb-5" style="text-align: center; color: #2e4259;"><strong>Cursos <span style="color: #3377ff;">Recomendados Para Ti</span></strong></h5>';
                echo '<div class="woocommerce productosRecomendados">';
                echo '<ul class="owl-carousel products productosRecomendados">';
                while ($loop->have_posts()) : $loop->the_post();
                    do_action('woocommerce_shop_loop');
                    wc_get_template_part('content', 'product');
                endwhile;
                ...
                echo '<div style="text-align: center;"><a class="btn btn-primary" style="background-color: #00bcff; border: #00BCFF;" title="Ver todos los cursos" href="/cursos/">VER TODOS LOS CURSOS</a></div>';
            }
        }
    }
