...
function api_isil_complete($order_id, $membresia = false, $persona = false)
{
    ...
    if (is_object($response) && isset($response->access_token) && $response->access_token != '') {
	    ...
	    $descuentos = $order->get_coupon_codes();
		if($descuentos){
			$coupon = new WC_Coupon($descuentos[0]);
			$coupon_code = $coupon->get_code();
			$coupon_desc = !empty($coupon->get_description()) ? $coupon->get_description() : $coupon_code;
		} else {
			$coupon_code = '';
			$coupon_desc = '';
		}
...
//	    }
        foreach ($items as $item) {
			...
			$detail =
				array(
					...
					//"discount_description" => $coupon_code
				);
       ...
       if(get_post_meta($order_id, 'billing_ciudad')[0]){
	    ...
	    $context = array( 'source' => 'salesforce_purchase' );
	    $logger->info($order_id . ' : ' . json_encode($cuerpo, JSON_PRETTY_PRINT), $context);
